...
WindowedMultipole::WindowedMultipole(hid_t group)
{
  ...
  // wmp capture 
  array<int, 2> version;
  if (get_num_datasets(group) == 9){
    read_dataset(group, "version", version);
    wmp_version_major_ = version[0];
    wmp_version_minor_ = version[1];
  } else {
    wmp_version_major_ = WMP_VERSION[0];
	wmp_version_minor_ = 1;
  }
  if (wmp_version_minor_ == 1)
    fissionable_ = (n_residues == 3);
  else if (wmp_version_minor_ == 2)
	fissionable_ = (n_residues == 4);
  else if (wmp_version_minor_ == 3)
	fissionable_ = (n_residues == 6);
  else {
	fissionable_ = (n_residues == 3);
    std::cout<<"Unknown Minor Version!"<<std::endl;
  }
...
std::tuple<double, double, double, double, double, double> WindowedMultipole::evaluate(
  double E, double sqrtkT) const
{
  ...
  if (sqrtkT > 0.0 && window.broaden_poly) {
    ...
    for (int i_poly = 0; i_poly < fit_order_ + 1; ++i_poly) {
      ...
      if (wmp_version_minor_ == 2)
	    sig_c += 
	      curvefit_(i_window, i_poly, FIT_C) * broadened_polynomials[i_poly];
	  if (wmp_version_minor_ == 3){
	    sig_c += 
	      curvefit_(i_window, i_poly, FIT_C) * broadened_polynomials[i_poly];
		sig_p += 
	      curvefit_(i_window, i_poly, FIT_P) * broadened_polynomials[i_poly];
		sig_l += 
	      curvefit_(i_window, i_poly, FIT_L) * broadened_polynomials[i_poly];
	  }	  
	  if (fissionable_) {
		if (wmp_version_minor_ == 1) 
	      sig_f += 
	        curvefit_(i_window, i_poly, FIT_F - 3) * broadened_polynomials[i_poly];
        else if (wmp_version_minor_ == 2) 
		  sig_f +=
            curvefit_(i_window, i_poly, FIT_F - 2) * broadened_polynomials[i_poly];
		else if (wmp_version_minor_ == 3)
          sig_f +=
    ...
    for (int i_poly = 0; i_poly < fit_order_ + 1; ++i_poly) {
	  ...
	  if (wmp_version_minor_ == 2)
	    sig_c += curvefit_(i_window, i_poly, FIT_C) * temp;
	  if (wmp_version_minor_ == 3){
	    sig_c += curvefit_(i_window, i_poly, FIT_C) * temp;
	    sig_p += curvefit_(i_window, i_poly, FIT_P) * temp;
		sig_l += curvefit_(i_window, i_poly, FIT_L) * temp;
	  }
      if (fissionable_) {
		if (wmp_version_minor_ == 1)
		  sig_f += curvefit_(i_window, i_poly, FIT_F - 3) * temp;
	    else if (wmp_version_minor_ == 2)
          sig_f += curvefit_(i_window, i_poly, FIT_F - 2) * temp;
		else if (wmp_version_minor_ == 3)
  ...
  if (sqrtkT == 0.0) {
    ...
    for (int i_pole = window.index_start; i_pole <= window.index_end;
	  ...
	  if (wmp_version_minor_ == 2)
	    sig_c += (data_(i_pole, MP_RC) * c_temp).real();
	  if (wmp_version_minor_ == 3){
	    sig_c += (data_(i_pole, MP_RC) * c_temp).real();
	    sig_p += (data_(i_pole, MP_RP) * c_temp).real();
		sig_l += (data_(i_pole, MP_RL) * c_temp).real();
	  }
      if (fissionable_) {
		if (wmp_version_minor_ == 1)
          sig_f += (data_(i_pole, MP_RF - 3) * c_temp).real();
		else if (wmp_version_minor_ == 2)
          sig_f += (data_(i_pole, MP_RF - 2) * c_temp).real();
	    else if (wmp_version_minor_ == 3)
    ...
    for (int i_pole = window.index_start; i_pole <= window.index_end;
	  ...
	  if (wmp_version_minor_ == 2)
	    sig_c += (data_(i_pole, MP_RC) * w_val).real();
	  if (wmp_version_minor_ == 3){
	    sig_c += (data_(i_pole, MP_RC) * w_val).real();
	    sig_p += (data_(i_pole, MP_RP) * w_val).real();
		sig_l += (data_(i_pole, MP_RL) * w_val).real();
	  }
      if (fissionable_) {
		if (wmp_version_minor_ == 1)
          sig_f += (data_(i_pole, MP_RF - 3) * w_val).real();
	    else if (wmp_version_minor_ == 2)
		  sig_f += (data_(i_pole, MP_RF - 2) * w_val).real();
	    else if (wmp_version_minor_ == 3)
  ...
  if (wmp_version_minor_ == 1) 
	sig_c = sig_a - sig_f;
  return std::make_tuple(sig_s, sig_a, sig_c, sig_p, sig_l, sig_f);
}

std::tuple<double, double, double, double, double, double> WindowedMultipole::evaluate_deriv(
  double E, double sqrtkT) const
{
  // ==========================================================================
  // Bookkeeping
